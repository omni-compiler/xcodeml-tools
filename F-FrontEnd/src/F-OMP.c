...
void check_for_OMP_pragma(expr x)
{
    ...
    if (CTL_TYPE(ctl_top) == CTL_OMP &&
        ...
        if (CTL_OMP_ARG_DIR(ctl_top) == OMP_F_PARALLEL_DO) {
            ...
            CTL_BLOCK(ctl_top) = OMP_FOR_pragma_list(
    ...
    switch (EXPR_INT(dir)) {
        ...
        case OMP_F_END_MASTER:
            if (CTL_TYPE(ctl_top) == CTL_OMP &&
                ...
                CTL_BLOCK(ctl_top) = OMP_pragma_list(OMP_MASTER, list0(LIST),
                                                     CURRENT_STATEMENTS);
                EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_OMP_ARG(ctl_top));
                pop_ctl();
            } else
        ...
        case OMP_F_END_ORDERED:
            if (CTL_TYPE(ctl_top) == CTL_OMP &&
                ...
                CTL_BLOCK(ctl_top) = OMP_pragma_list(OMP_ORDERED, list0(LIST),
                                                     CURRENT_STATEMENTS);
                EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_OMP_ARG(ctl_top));
                pop_ctl();
            } else
        ...
        case OMP_F_END_CRITICAL:
            if (CTL_TYPE(ctl_top) == CTL_OMP &&
                ...
                CTL_BLOCK(ctl_top) = OMP_pragma_list(
                    OMP_CRITICAL, EXPR_ARG2(CTL_OMP_ARG(ctl_top)),
                    CURRENT_STATEMENTS);
                EXPR_LINE(CTL_BLOCK(ctl_top)) = EXPR_LINE(CTL_OMP_ARG(ctl_top));
                pop_ctl();
            } else
    ...
    FOR_ITEMS_IN_LIST (lp, x) {
        ...
        switch (EXPR_INT(EXPR_ARG1(c))) {
            case OMP_DATA_DEFAULT: /* default(shared|none|private) */
                ...
                pclause =
                    list_put_last(pclause, list2(LIST, EXPR_ARG1(c),
            ...
            case OMP_DIR_IF:
                ...
                if (pragma == OMP_PARALLEL) {
                    ...
                    pclause =
                        list_put_last(pclause, list2(LIST, EXPR_ARG1(c), v));
                } else if (pragma == OMP_TASK) {
                    v = compile_expression(EXPR_ARG2(c));
                    dclause =
                        list_put_last(dclause, list2(LIST, EXPR_ARG1(c), v));
                }
                break;
            ...
            case OMP_DIR_NUM_THREADS:
                if (!is_parallel) {
                    error_at_node(x,
                                  "'num_threads' clause must be in PARALLEL");
                    break;
                }
                v = compile_expression(EXPR_ARG2(c));
            ...
            case OMP_DATA_LASTPRIVATE:
                ...
                if (pragma != OMP_FOR && pragma != OMP_SECTIONS) {
                    error_at_node(x, "'lastprivate' clause must be in "
                                     "FOR or SECTIONS");
                    break;
                }
                dclause = list_put_last(dclause, c);
            ...
            case OMP_DATA_REDUCTION_MAX:
                ...
                if (pragma == OMP_PARALLEL)
                    ...
                    error_at_node(x,
                                  "'reduction' clause must not be in SINGLE");
                break;
...
ret:
    ...
    if (is_parallel && (pragma != OMP_PARALLEL))
        dclause = list_put_last(
            dclause,
            list2(LIST, make_enode(INT_CONSTANT, ((void *)OMP_DIR_NOWAIT)),
                  NULL));
    *pc = pclause;
    *dc = dclause;
}
...
void compile_OMP_name_list(expr x)
{
    ...
    FOR_ITEMS_IN_LIST (lp, x) {
        ...
        if (EXPR_CODE(v) == LIST) {
            ...
            v = EXPR_ARG1(v);
            if (EXPR_CODE(v) != IDENT) {
                error("common name is expected in list of OpenMP "
                      "directive");
                continue;
            }
            ...
            if ((id = find_common_ident(EXPR_SYM(v))) == NULL) {
                error("common name '%s' in OpenMP directive is "
                      "undefined",
                      SYM_NAME(EXPR_SYM(v)));
                continue;
            }
        ...
        if (ID_CLASS(id) != CL_VAR) {
            if (ID_CLASS(id) == CL_UNKNOWN)
                ...
                error("identifier '%s' is not a variable in OpenMP "
                      "directive",
                      ID_NAME(id));
                continue;
            }
        ...
        if (strcmp(s, ss) == 0) {
            ...
            if ((tp = ID_TYPE(id)) != NULL) {
                if (TYPE_BASIC_TYPE(tp) != t)
                    warning("data type of OpenMP runtime function "
                            "'%s' may be "
                            "wrong",
                            s);
            } else
