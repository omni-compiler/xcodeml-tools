...
#define NAME_CLASS_NAMES                                                       \
    {                                                                          \
        "CL_UNKNOWN", "CL_PARAM", "CL_VAR", "CL_ENTRY", "CL_MAIN",             \
            "CL_MODULE", "CL_CONTAINS", "CL_BLOCK", "CL_PROC", "CL_LABEL",     \
            "CL_FORMAT", "CL_TAGNAME", "CL_NAMELIST", "CL_COMMON",             \
            "CL_ELEMENT", "CL_GENERICS", "CL_TYPE_PARAM",                      \
            "CL_TYPE_BOUND_PROCS", "CL_MULTI", "CL_ENUM",                      \
    }
...
#define PROC_CLASS_NAMES                                                       \
    {                                                                          \
        "P_UNKNOWN", "P_EXTERNAL", "P_INTRINSIC", "P_STFUNCT", "P_THISPROC",   \
            "P_DEFINEDPROC", "P_UNDEFINEDPROC", "P_EXT_DEFINEDPROC"            \
    }
...
#define STORAGE_CLASS_NAMES                                                    \
    {                                                                          \
        "STG_UNKNOWN", "STG_ARG", "STG_AUTO", "STG_SAVE", "STG_EXT",           \
            "STG_COMMON", "STG_EQUIV", "STG_COMEQ", "STG_TAGNAME", "STG_NONE", \
            "STG_TYPE_PARAM", "STG_INDEX",                                     \
    }
...
#define VAR_INIT_EQUIV 4
            ...
            struct ident_descriptor *ref_proc; /* for a procedure variable,
                                                  refer to a procedure name */
        } var_info;
        struct {
            LABEL_TYPE lab_type;
            char lab_blklevel;
...
#define ID_MAY_HAVE_ACCECIBILITY(id)                                           \
    (ID_STORAGE(id) != STG_ARG &&                                              \
     (ID_CLASS(id) == CL_UNKNOWN || ID_CLASS(id) == CL_PARAM ||                \
      ID_CLASS(id) == CL_VAR || ID_CLASS(id) == CL_ENTRY ||                    \
      ID_CLASS(id) == CL_PROC || ID_CLASS(id) == CL_TAGNAME ||                 \
      ID_CLASS(id) == CL_NAMELIST || ID_CLASS(id) == CL_GENERICS))
...
#define SAFE_FOREACH_ID(ip, iq, headp) SAFE_FOREACH(ip, iq, headp, ID_NEXT)
...
#define ID_IS_TYPE_PARAM(id) ((ID_CLASS((id)) == CL_TYPE_PARAM))
...
#define EXT_PROC_CONT_EXT_SYMS(ep)                                             \
    ((ep)->info.proc_info.contains_external_symbols)
#define EXT_PROC_CONT_EXT_LINE(ep) ((ep)->info.proc_info.contains_line)
#define EXT_PROC_INTERFACES(ep)                                                \
...
#define EXT_PROC_IS_INTERFACE_DEF(ep) (EXT_PROC_CLASS(ep) == EP_INTERFACE_DEF)
...
#define SAFE_FOREACH_EXT_ID(ep, eq, headp) SAFE_FOREACH(ep, eq, headp, EXT_NEXT)
