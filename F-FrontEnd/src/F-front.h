...
#define SAFE_FOREACH(p, q, list, iter)                                         \
    for ((p) = (list), (q) = (p) ? iter(p) : NULL; (p) != NULL;                \
         (p) = (q), (q) = (p) ? iter(q) : NULL)
...
#define CONTROL_TYPE_NAMES                                                     \
    {                                                                          \
        "CTL_NONE", "CTL_DO", "CTL_IF", "CTL_ELSE", "CTL_WHERE",               \
            "CTL_ELSE_WHERE", "CTL_SELECT", "CTL_CASE", "CTL_STRUCT",          \
            "CTL_OMP", "CTL_XMP", "CTL_ACC", "CTL_CRITICAL", "CTL_BLK",        \
            "CTL_INTERFACE", "CTL_FORALL", "CTL_ENUM", "CTL_ASSOCIATE",        \
    }
...
#define FOR_CTLS(cp)                                                           \
    for (cp = ctl_base; cp != ctl_top && (CTL_NEXT(cp) != NULL &&              \
                                          CTL_TYPE(CTL_NEXT(cp)) != CTL_NONE); \
         cp = CTL_NEXT(cp))
...
#define UNIT_CTL_LOCAL_COMMON_SYMBOLS(u)                                       \
    ((UNIT_CTL_LOCAL_ENV(u))->common_symbols)
#define UNIT_CTL_LOCAL_LABELS(u) ((UNIT_CTL_LOCAL_ENV(u))->labels)
#define UNIT_CTL_LOCAL_EXTERNAL_SYMBOLS(u)                                     \
    ((UNIT_CTL_LOCAL_ENV(u))->external_symbols)
...
extern int check_in_interface _ANSI_ARGS_((void));
...
extern void set_implicit_type_uc _ANSI_ARGS_((UNIT_CTL uc, TYPE_DESC tp, int c1,
                                              int c2,
                                              int ignore_declared_flag));
extern void set_implicit_type _ANSI_ARGS_((TYPE_DESC tp, int c1, int c2));
extern void set_implicit_storage_uc _ANSI_ARGS_((UNIT_CTL uc,
                                                 enum storage_class stg, int c1,
                                                 int c2));
extern void set_implicit_storage _ANSI_ARGS_((enum storage_class stg, int c1,
                                              int c2));
...
extern int compile_type_param_values_dummy _ANSI_ARGS_((TYPE_DESC struct_type,
                                                        expr type_param_args));
extern int compile_type_param_values _ANSI_ARGS_((TYPE_DESC struct_type,
                                                  expr type_param_args,
                                                  expv type_param_values,
                                                  ID *used));
...
extern void declare_statement_function _ANSI_ARGS_((ID id, expr args,
                                                    expr body));
...
extern expv compile_function_call_check_intrinsic_arg_type
    _ANSI_ARGS_((ID f_id, expr args, int ignoreTypeMismatch));
extern expv compile_highorder_function_call _ANSI_ARGS_((ID f_id, expr args,
                                                         int isCall));

extern int type_param_values_required _ANSI_ARGS_((TYPE_DESC struct_type));
extern expv compile_struct_constructor _ANSI_ARGS_((ID struct_id,
                                                    expr type_param_argss,
                                                    expr args));
extern TYPE_DESC type_apply_type_parameter
    _ANSI_ARGS_((TYPE_DESC tp, expv type_param_values));
...
extern codims_desc *compile_codimensions _ANSI_ARGS_((expr dims,
                                                      int is_in_alloc));
...
extern ID find_struct_member_allow_private _ANSI_ARGS_(
    (TYPE_DESC struct_td, SYMBOL sym, int allow_private, int allow_parent));
...
extern EXT_ID declare_external_proc_id _ANSI_ARGS_((SYMBOL s, TYPE_DESC tp,
                                                    int def_flag));
extern EXT_ID declare_external_id _ANSI_ARGS_((SYMBOL s, enum storage_class tag,
                                               int def_flag));
...
extern void update_type_bound_procedures_forall
    _ANSI_ARGS_((TYPE_DESC struct_decls, ID local_symbols));
extern int type_bound_procedure_type_match _ANSI_ARGS_((EXT_ID f1, EXT_ID f2,
                                                        int has_pass_arg));
...
extern int type_is_strict_compatible _ANSI_ARGS_((TYPE_DESC tp, TYPE_DESC tq,
                                                  int compare_rank,
                                                  int issue_error));
extern int type_is_compatible_for_assignment _ANSI_ARGS_((TYPE_DESC tp1,
                                                          TYPE_DESC tp2));
extern int type_is_compatible_for_allocation _ANSI_ARGS_((TYPE_DESC left,
                                                          TYPE_DESC right));
...
extern int type_is_specific_than _ANSI_ARGS_((TYPE_DESC tp1, TYPE_DESC tp2));
extern void function_type_udpate _ANSI_ARGS_((TYPE_DESC ftp, ID idList));
extern int function_type_is_appliable _ANSI_ARGS_((TYPE_DESC ftp, expv args,
                                                   int issue_error));
extern int function_type_is_compatible _ANSI_ARGS_((const TYPE_DESC ftp1,
                                                    const TYPE_DESC ftp2));
...
extern int procedure_has_pass_arg _ANSI_ARGS_((const TYPE_DESC ftp,
                                               const SYMBOL pass_arg,
                                               const TYPE_DESC stp));
extern int procedure_is_assignable _ANSI_ARGS_((const TYPE_DESC left,
                                                const TYPE_DESC right));

extern void replace_or_assign_type _ANSI_ARGS_((TYPE_DESC * tp,
                                                const TYPE_DESC new_tp));

extern int are_dimension_and_shape_conformant_by_type
    _ANSI_ARGS_((expr x, TYPE_DESC lt, TYPE_DESC rt, expv *shapePtr,
                 int for_argument, int issue_error));

extern TYPE_DESC get_binary_numeric_intrinsic_operation_type(TYPE_DESC t0,
                                                             TYPE_DESC t1);
extern TYPE_DESC get_binary_comparative_intrinsic_operation_type(TYPE_DESC t0,
                                                                 TYPE_DESC t1);
extern TYPE_DESC get_binary_equal_intrinsic_operation_type(TYPE_DESC t0,
                                                           TYPE_DESC t1);
...
extern expv compile_array_ref _ANSI_ARGS_((ID id, expv ary, expr args,
                                           int isLeft));
...
extern expv expv_complex_op _ANSI_ARGS_((enum expr_code op, TYPE_DESC tp,
...
extern expv expv_cons _ANSI_ARGS_((enum expr_code code, TYPE_DESC tp, expv left,
                                   expv right));
extern expv expv_user_def_cons _ANSI_ARGS_((enum expr_code code, TYPE_DESC tp,
                                            expv id, expv left, expv right));
...
extern expv expv_float_term _ANSI_ARGS_((enum expr_code code, TYPE_DESC tp,
                                         omldouble_t d, const char *token));
...
extern expr list4 _ANSI_ARGS_((enum expr_code code, expr x1, expr x2, expr x3,
                               expr x4));
extern expr list5 _ANSI_ARGS_((enum expr_code code, expr x1, expr x2, expr x3,
                               expr x4, expr x5));
extern expr list6 _ANSI_ARGS_((enum expr_code code, expr x1, expr x2, expr x3,
                               expr x4, expr x5, expr x6));

extern expr expr_list_get_n _ANSI_ARGS_((expr x, int n));
extern expr expr_list_get_n_or_named _ANSI_ARGS_((expr x, int n,
                                                  const char *named_arg));
extern int expr_list_set_n _ANSI_ARGS_((expr x, int n, expr val,
                                        int doOverride));
...
extern expr make_float_enode _ANSI_ARGS_((enum expr_code code, omldouble_t d,
                                          const char *token));
...
extern EXT_ID new_external_id_for_external_decl _ANSI_ARGS_((SYMBOL sp,
                                                             TYPE_DESC tp));
...
extern void compile_DATA_decl_or_statement _ANSI_ARGS_((expr x,
                                                        int is_declaration));
...
extern expr GetIoSpecifierValue _ANSI_ARGS_((expv v, int id,
                                             int *specifiedPtr));
...
extern expv GetIoSpecifierValueAsInteger _ANSI_ARGS_((expv ioSpec, int id,
                                                      int doAddr,
                                                      int *haveItPtr));
extern expv GetIoSpecifierValueAsIntegerVariable
    _ANSI_ARGS_((expv ioSpec, int id, int doAddr, int *haveItPtr, ID *vIdPtr));
extern expv GetIoSpecifierValueAsLogicalVariable
    _ANSI_ARGS_((expv ioSpec, int id, int doAddr, int *haveItPtr, ID *vIdPtr));
extern expv GetIoSpecifierValueAsString _ANSI_ARGS_((expv ioSpec, int id,
                                                     int *haveItPtr,
                                                     expv *lenVPtr));
extern expv GetIoSpecifierValueAsStringVariable _ANSI_ARGS_(
    (expv ioSpec, int id, int *haveItPtr, expv *lenVPtr, ID *vIdPtr));
...
extern void begin_submodule _ANSI_ARGS_((expr name, expr module_name,
                                         expr submodule_name));
...
extern TYPE_DESC copy_type_partially(TYPE_DESC tp, int doCopyAttr);
extern int type_is_omissible(TYPE_DESC tp, uint32_t attr, uint32_t ext);
