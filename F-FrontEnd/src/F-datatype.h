...
#define BASIC_TYPE_NAMES                                                       \
    {                                                                          \
        "*undef*", "integer", "real", "double_real", "complex",                \
            "double_complex", "logical", "character", "subroutine", "*array*", \
            "*function*", "*type*", "*numeric*", "*numeric_all*", "module",    \
            "*generic*", "*namelist*", "*comforms_to_the_lefthand*",           \
    }
...
#define TYPE_ATTR_ABSTRACT                                                     \
    0x10000000 /* for abstract derived-type or abstract interface */
...
#define TYPE_EXFLAGS_USED_EXPLICIT                                             \
    0x00000004 /* type is used explicitly (used for function/subroutine call)  \
                */
#define TYPE_EXFLAGS_NOT_FIXED                                                 \
    0x00000008 /* type is not fixed, since expression                          \
                  contains undefined function. */
#define TYPE_EXFLAGS_FOR_FUNC_SELF                                             \
    0x00000010 /* type is for the function itself */
...
typedef struct type_descriptor {
    ...
    struct ident_descriptor
        *parent; /* represents super-class of this derived type.  */
    struct ident_descriptor
        *type_parameters; /* type parameters for derived type */
    /* For the parameterized derived-type, it works as dummy arguments */
    /* For the instance of the parameterized derived-type, it works as actual
     * arguments */
    expv type_param_values; /* type parameter values */
    /* struct ident_descriptor *type_parameters_used; /\* TODO: write nice
     * comment *\/ */
    ...
    struct {
        ...
        int is_defined;  /* function/subroutine has a definition (For submodule
                            only) */
        ...
        struct ident_descriptor
            *pass_arg; /* for the function type of procedure variable OR
                          type-bound procedure */
        struct type_descriptor
            *pass_arg_type; /* for the function type of procedure variable OR
                               type-bound procedure */

        struct {
            struct ident_descriptor
                *generics; /* for the function type of type-bound generic */
...
#define TYPE_SET_PARAMETER(tp)                                                 \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_PARAMETER)
#define TYPE_UNSET_PARAMETER(tp)                                               \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_PARAMETER)
#define TYPE_IS_ALLOCATABLE(tp)                                                \
    ((tp)->attr.type_attr_flags & TYPE_ATTR_ALLOCATABLE)
#define TYPE_SET_ALLOCATABLE(tp)                                               \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_ALLOCATABLE)
#define TYPE_UNSET_ALLOCATABLE(tp)                                             \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_ALLOCATABLE)
...
#define TYPE_UNSET_EXTERNAL(tp)                                                \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_EXTERNAL)
#define TYPE_IS_INTRINSIC(tp) ((tp)->attr.type_attr_flags & TYPE_ATTR_INTRINSIC)
#define TYPE_SET_INTRINSIC(tp)                                                 \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_INTRINSIC)
#define TYPE_UNSET_INTRINSIC(tp)                                               \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_INTRINSIC)
...
#define TYPE_UNSET_OPTIONAL(tp)                                                \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_OPTIONAL)
...
#define TYPE_UNSET_POINTER(tp)                                                 \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_POINTER)
...
#define TYPE_UNSET_PRIVATE(tp)                                                 \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_PRIVATE)
#define TYPE_IS_PROTECTED(tp) ((tp)->attr.type_attr_flags & TYPE_ATTR_PROTECTED)
#define TYPE_SET_PROTECTED(tp)                                                 \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_PROTECTED)
#define TYPE_UNSET_PROTECTED(tp)                                               \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_PROTECTED)
...
#define TYPE_UNSET_SEQUENCE(tp)                                                \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_SEQUENCE)
#define TYPE_IS_INTERNAL_PRIVATE(tp)                                           \
    ((tp)->attr.type_attr_flags & TYPE_ATTR_INTERNAL_PRIVATE)
#define TYPE_SET_INTERNAL_PRIVATE(tp)                                          \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_INTERNAL_PRIVATE)
#define TYPE_UNSET_INTERNAL_PRIVATE(tp)                                        \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_INTERNAL_PRIVATE)
#define TYPE_IS_RECURSIVE(tp) ((tp)->attr.type_attr_flags & TYPE_ATTR_RECURSIVE)
#define TYPE_SET_RECURSIVE(tp)                                                 \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_RECURSIVE)
#define TYPE_UNSET_RECURSIVE(tp)                                               \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_RECURSIVE)
...
#define TYPE_SET_ELEMENTAL(tp)                                                 \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_ELEMENTAL)
#define TYPE_UNSET_ELEMENTAL(tp)                                               \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_ELEMENTAL)
...
#define TYPE_SET_PROCEDURE(tp)                                                 \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_PROCEDURE)
#define TYPE_UNSET_PROCEDURE(tp)                                               \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_PROCEDURE)
#define TYPE_IS_INTENT_IN(tp) ((tp)->attr.type_attr_flags & TYPE_ATTR_INTENT_IN)
#define TYPE_SET_INTENT_IN(tp)                                                 \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_INTENT_IN)
#define TYPE_UNSET_INTENT_IN(tp)                                               \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_INTENT_IN)
#define TYPE_IS_INTENT_OUT(tp)                                                 \
    ((tp)->attr.type_attr_flags & TYPE_ATTR_INTENT_OUT)
#define TYPE_SET_INTENT_OUT(tp)                                                \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_INTENT_OUT)
#define TYPE_UNSET_INTENT_OUT(tp)                                              \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_INTENT_OUT)
#define TYPE_IS_INTENT_INOUT(tp)                                               \
    ((tp)->attr.type_attr_flags & TYPE_ATTR_INTENT_INOUT)
#define TYPE_SET_INTENT_INOUT(tp)                                              \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_INTENT_INOUT)
#define TYPE_UNSET_INTENT_INOUT(tp)                                            \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_INTENT_INOUT)
...
#define TYPE_UNSET_VOLATILE(tp)                                                \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_VOLATILE)
...
#define TYPE_IS_ASYNCHRONOUS(tp)                                               \
    ((tp)->attr.type_attr_flags & TYPE_ATTR_ASYNCHRONOUS)
#define TYPE_SET_ASYNCHRONOUS(tp)                                              \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_ASYNCHRONOUS)
#define TYPE_UNSET_ASYNCHRONOUS(tp)                                            \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_ASYNCHRONOUS)
...
#define TYPE_UNSET_ABSTRACT(tp)                                                \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_ABSTRACT)
#define TYPE_IS_CONTIGUOUS(tp)                                                 \
    ((tp)->attr.type_attr_flags & TYPE_ATTR_CONTIGUOUS)
#define TYPE_SET_CONTIGUOUS(tp)                                                \
    ((tp)->attr.type_attr_flags |= TYPE_ATTR_CONTIGUOUS)
#define TYPE_UNSET_CONTIGUOUS(tp)                                              \
    ((tp)->attr.type_attr_flags &= ~TYPE_ATTR_CONTIGUOUS)
...
#define TYPE_UNSET_OVERRIDDEN(tp)                                              \
    ((tp)->attr.exflags &= ~TYPE_EXFLAGS_OVERRIDDEN)
#define TYPE_IS_USED_EXPLICIT(tp)                                              \
    ((tp)->attr.exflags & TYPE_EXFLAGS_USED_EXPLICIT)
#define TYPE_SET_USED_EXPLICIT(tp)                                             \
    ((tp)->attr.exflags |= TYPE_EXFLAGS_USED_EXPLICIT)
#define TYPE_UNSET_USED_EXPLICIT(tp)                                           \
    ((tp)->attr.exflags &= ~TYPE_EXFLAGS_USED_EXPLICIT)
...
#define TYPE_IS_FOR_FUNC_SELF(tp)                                              \
    ((tp)->attr.exflags & TYPE_EXFLAGS_FOR_FUNC_SELF)
#define TYPE_SET_FOR_FUNC_SELF(tp)                                             \
    ((tp)->attr.exflags |= TYPE_EXFLAGS_FOR_FUNC_SELF)
#define TYPE_UNSET_FOR_FUNC_SELF(tp)                                           \
    ((tp)->attr.exflags &= ~TYPE_EXFLAGS_FOR_FUNC_SELF)

#define TYPE_IS_UNCHANGABLE(tp) ((tp)->attr.exflags & TYPE_EXFLAGS_UNCHANGABLE)
#define TYPE_SET_UNCHANGABLE(tp)                                               \
    ((tp)->attr.exflags |= TYPE_EXFLAGS_UNCHANGABLE)
#define TYPE_UNSET_UNCHANGABLE(tp)                                             \
    ((tp)->attr.exflags &= ~TYPE_EXFLAGS_UNCHANGABLE)
...
#define TYPE_ATTR_FOR_COMPARE                                                  \
    (TYPE_ATTR_PARAMETER | TYPE_ATTR_ALLOCATABLE | TYPE_ATTR_INTRINSIC |       \
     TYPE_ATTR_OPTIONAL | TYPE_ATTR_POINTER | TYPE_ATTR_TARGET |               \
     TYPE_ATTR_INTENT_IN | TYPE_ATTR_INTENT_OUT | TYPE_ATTR_INTENT_INOUT |     \
     TYPE_ATTR_SEQUENCE | TYPE_ATTR_VOLATILE | TYPE_ATTR_CLASS |               \
     TYPE_ATTR_BIND | TYPE_ATTR_VALUE)

#define TYPE_ATTRS_SUBOBJECT_PROPAGATE                                         \
    (TYPE_ATTR_POINTER | TYPE_ATTR_TARGET | TYPE_ATTR_VOLATILE |               \
     TYPE_ATTR_ASYNCHRONOUS | TYPE_ATTR_ALLOCATABLE | TYPE_ATTR_PROTECTED)
...
#define TYPE_EXTATTRS_SUBOBJECT_PROPAGATE (TYPE_EXFLAGS_READONLY)
...
#define TYPE_HAS_NON_ACCESSIBILITY_FLAGS(tp)                                   \
    (TYPE_ATTR_FLAGS(tp) != 0 && (TYPE_ATTR_FLAGS(tp) != TYPE_ATTR_PRIVATE) && \
     (TYPE_ATTR_FLAGS(tp) != TYPE_ATTR_PUBLIC) &&                              \
     (TYPE_ATTR_FLAGS(tp) != TYPE_ATTR_PROTECTED))
...
#define TYPE_HAVE_KIND(tp) ((tp) != NULL && TYPE_KIND(tp) != NULL)
...
#define IS_COMPLEX(tp)                                                         \
    ((tp) != NULL && (TYPE_BASIC_TYPE(tp) == TYPE_COMPLEX ||                   \
                      TYPE_BASIC_TYPE(tp) == TYPE_DCOMPLEX))
#define IS_REAL(tp)                                                            \
    ((tp) != NULL &&                                                           \
     (TYPE_BASIC_TYPE(tp) == TYPE_REAL || TYPE_BASIC_TYPE(tp) == TYPE_DREAL))
#define IS_INT(tp) ((tp) != NULL && (TYPE_BASIC_TYPE(tp) == TYPE_INT))
#define IS_INT_OR_REAL(tp) (IS_INT(tp) || IS_REAL(tp) || IS_GNUMERIC(tp))
#define IS_NUMERIC(tp)                                                         \
    ((tp) != NULL && (IS_COMPLEX(tp) || IS_REAL(tp) || IS_INT(tp) ||           \
                      IS_GNUMERIC(tp) || IS_GNUMERIC_ALL(tp)))
#define IS_NUMERIC_OR_LOGICAL(tp) (IS_NUMERIC(tp) || IS_LOGICAL(tp))
...
#define IS_INT_PARAM_V(v) (IS_INT(EXPV_TYPE(v)) && expr_is_param(v))
...
#define IS_GNUMERIC(tp) ((tp) != NULL && (TYPE_BASIC_TYPE(tp) == TYPE_GNUMERIC))
...
#define IS_DOUBLED_TYPE(tp)                                                    \
    ((tp) != NULL && (TYPE_BASIC_TYPE(tp) == TYPE_DREAL ||                     \
                      TYPE_BASIC_TYPE(tp) == TYPE_DCOMPLEX))
#define IS_MODULE(tp) ((tp) != NULL && TYPE_BASIC_TYPE(tp) == TYPE_MODULE)

#define IS_NAMELIST(tp) ((tp) != NULL && TYPE_BASIC_TYPE(tp) == TYPE_NAMELIST)

#define IS_REFFERENCE(tp)                                                      \
    ((tp) != NULL && TYPE_N_DIM(tp) == 0 && TYPE_REF(tp) != NULL)

#define IS_ENUM(tp) ((tp) != NULL && TYPE_BASIC_TYPE(tp) == TYPE_ENUM)

/* represents CLASS(*) */
#define IS_ANY_CLASS(tp)                                                       \
    ((tp) != NULL && TYPE_IS_CLASS(tp) && TYPE_REF(tp) == NULL)

#define TYPE_IS_MODIFIED(tp) ((tp) != NULL && (tp)->is_modified)

#define SET_MODIFIED(tp) ((tp != NULL) && ((tp)->is_modified = TRUE))

#define UNSET_MODIFIED(tp) ((tp != NULL) && ((tp)->is_modified = FALSE))
...
#define FUNCTION_TYPE_HAS_EXPLICIT_ARGS(tp)                                    \
    ((tp)->proc_info.has_explicit_arguments)
...
#define FUNCTION_TYPE_HAS_EXPLICT_INTERFACE(tp)                                \
    (FUNCTION_TYPE_RETURN_TYPE(tp) != NULL &&                                  \
     FUNCTION_TYPE_HAS_EXPLICIT_ARGS(tp))
...
/* For is_external attribute */
#define FUNCTION_TYPE_IS_MOUDLE_PROCEDURE(tp)                                  \
    ((tp)->proc_info.is_module_procedure == TRUE)
#define FUNCTION_TYPE_SET_MOUDLE_PROCEDURE(tp)                                 \
    ((tp)->proc_info.is_module_procedure = TRUE)
#define FUNCTION_TYPE_UNSET_MOUDLE_PROCEDURE(tp)                               \
    ((tp)->proc_info.is_module_procedure = FALSE)

#define FUNCTION_TYPE_IS_VISIBLE_INTRINSIC(tp)                                 \
    ((tp)->proc_info.is_visible_intrinsic == TRUE)
#define FUNCTION_TYPE_SET_VISIBLE_INTRINSIC(tp)                                \
    ((tp)->proc_info.is_visible_intrinsic = TRUE)
#define FUNCTION_TYPE_UNSET_VISIBLE_INTRINSIC(tp)                              \
    ((tp)->proc_info.is_visible_intrinsic = FALSE)

#define TYPE_BOUND_GENERIC_TYPE_GENERICS(tp)                                   \
    ((tp)->proc_info.type_bound_proc_info.generics)
#define TYPE_BOUND_PROCEDURE_TYPE_HAS_PASS_ARG(tp)                             \
    ((tp)->proc_info.has_pass_arg)
