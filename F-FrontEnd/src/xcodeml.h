...
/* macros for a id tag. */
#define GET_ID_TYPE(ndPtr)                                                     \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "type"))
#define GET_ID_SCLASS(ndPtr)                                                   \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "sclass"))
#define GET_ID_NAME(ndPtr)                                                     \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "name"))

/* macros for useDecl/useOnlyDecl */
#define GET_LOCAL(ndPtr)                                                       \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "local_name"))
#define GET_USE(ndPtr)                                                         \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "use_name"))
...
#define GET_IS_PUBLIC(ndPtr)                                                   \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_public"))
#define GET_IS_PRIVATE(ndPtr)                                                  \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_private"))
#define GET_IS_POINTER(ndPtr)                                                  \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_pointer"))
#define GET_IS_TARGET(ndPtr)                                                   \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_target"))
#define GET_IS_EXTERNAL(ndPtr)                                                 \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_external"))
#define GET_IS_INTRINSIC(ndPtr)                                                \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_intrinsic"))
#define GET_IS_OPTIONAL(ndPtr)                                                 \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_optional"))
#define GET_IS_SAVE(ndPtr)                                                     \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_save"))
#define GET_IS_PARAMETER(ndPtr)                                                \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_parameter"))
#define GET_IS_ALLOCATABLE(ndPtr)                                              \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_allocatable"))
#define GET_IS_SEQUENCE(ndPtr)                                                 \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_sequence"))
#define GET_INTENT(ndPtr)                                                      \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "intent"))

/* macros for module tag */
#define GET_FMODULEDEFINITION(ndPtr)                                           \
    xcodeml_getByName((ndPtr), "FmoduleDefinition")
...
#define GET_MEMBER(ndPtr)                                                      \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "member"))
...
#define GET_IS_ASHAPE(ndPtr)                                                   \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_assumed_shape"))
#define GET_IS_ASIZE(ndPtr)                                                    \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_assumed_size"))

/* macros for interface */
#define GET_INTF_NAME(ndPtr)                                                   \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "name"))
#define GET_INTF_IS_OPERATOR(ndPtr)                                            \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_operator"))
#define GET_INTF_IS_ASSGIN(ndPtr)                                              \
    xcodeml_getAsBool(xcodeml_getByName((ndPtr), "is_assignment"))

/* macros for function */
#define GET_PARAMS(ndPtr) xcodeml_getByName((ndPtr), "params")
#define GET_RETURN(ndPtr)                                                      \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "return_type"))
#define GET_RESULT_NAME(ndPtr)                                                 \
    xcodeml_getAsString(xcodeml_getByName((ndPtr), "result_name"))
...
#define PRIV_PARM_LINK_FOR(lp, priv_parm_list)                                 \
    if (priv_parm_list_head != NULL)                                           \
        for (lp = priv_parm_list_head; lp != NULL; lp = PRIV_PARM_LINK(lp))
