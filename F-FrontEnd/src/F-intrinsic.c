...
expv compile_intrinsic_call0(ID id, expv args, int ignoreTypeMismatch)
{
                    ...
                    if (INTR_ARG_NAME(ep)[j]) {
                        if (strcmp(INTR_ARG_NAME(ep)[j], EXPV_KWOPT_NAME(a)) ==
                            ...
                            if (compare_intrinsic_arg_type(
                                ...
                                if (strcmp(INTR_ARG_NAME(ep)[j], "kin"
                                                                 "d") == 0) {
                                    kindV = a;
                                }
    ...
    if (found == 1) {
        ...
        if (INTR_RETURN_TYPE(ep) != INTR_TYPE_NONE) {
            ...
            if (!(isValidType(tp))) {
                // fatal("%s: can't determine return type.",
...
static void generate_contracted_shape_expr(TYPE_DESC tp, expr dimSpec, int dim)
{
    expv dimElm;
    ...
    if (INTR_RETURN_TYPE_SAME_AS(ep) >= 0) {
        ...
        switch (INTR_RETURN_TYPE_SAME_AS(ep)) {
            ...
            case -6 /* if not dynamic return type,
                ...
                if (!(INTR_IS_RETURN_TYPE_DYNAMIC(ep)) &&
                    ...
                    if (bType == TYPE_UNKNOWN) {
                        fatal("invalid intrinsic return type "
                              "(case -1/-6).");
                        /* not reached. */
                        return NULL;
                    } else {
                        if (kindV == NULL) {
                            ...
                            /*
                             * Don't use
                             * BASIC_TYPE_DESC(bType) very
                             * here, since we need to set a
                             * kind to the TYPE_DESC.
                             */
                    ...
                    switch (INTR_OP(ep)) {
                        ...
                        case INTR_COUNT: {
                            ...
                            if (EXPV_CODE(dim) == INT_CONSTANT) {
                                ...
                                generate_assumed_shape_expr(shape,
                                                            TYPE_N_DIM(tp) - 1);
                            }
                        } break;
                        ...
                        case INTR_SPREAD: {
                            ...
                            ncopies =
                                expr_list_get_n_or_named(args, 2, "ncopies");
                        ...
                        case INTR_RESHAPE: {
                            ...
                            /*
                             * We can't determine # of the
                             * elements in this array that
                             * represents dimension of the
                             * return type, which is identical
                             * to the reshaped array. In order
                             * to express this, we introduce a
                             * special TYPE_DESC, which is
                             * having a flag to specify that the
                             * type is generated by the
                             * reshape() intrinsic.
                             */

                            /*
                             * dummy one dimensional assumed
                             * array.
                             */
                        ...
                        case INTR_MATMUL: {
                             ...
                             *  instead of max_type()? I think
                             * so but not sure at this moment.
                             */
                            ...
                            if (TYPE_N_DIM(t1) == 2 && TYPE_N_DIM(t2) == 2) {
                                 ...
                                 */
                                shape =
                                    list2(LIST, EXPR_ARG1(s1), EXPR_ARG2(s2));
                        ...
                        case INTR_UCOBOUND:
                        case INTR_LCOBOUND: {
                            /* `THIS_IMAGE(COARRAY)` returns an
                               1-rank array. Its length is
                               euquals to the corank of COARRAY
                             */
                            int corank;
                            expv dims;
                            a = expr_list_get_n(args, 0);
                        ...
                        case INTR_MINLOC: {
                            /*
                             * `MAXLOC/MINLOC(ARRAY, DIM)`
                             * returns an 1-rank array. Its
                             * shape is [d_1, ..., d_dim-1,
                             * d_dim+1, ..., d_n]
                             */
            ...
            case -3: {
                ...
                /*
                 * FIXME:
                 *	No need to check kindV?? I believe we don't,
                 *though.
                 */
            ...
            case -5: {
                ...
                switch (INTR_OP(ep)) {
                    ...
                    case INTR_DIMAG: {
                        bType = get_basic_type(tp);
                        if (bType != TYPE_COMPLEX && bType != TYPE_DCOMPLEX) {
                            error("argument is not a complex "
                                  "type.");
                            return NULL;
                        }
                        bType =
                            (bType == TYPE_COMPLEX) ? TYPE_REAL : TYPE_DREAL;
                        break;
                    }
                    default: {
                        bType = intr_type_to_basic_type(INTR_RETURN_TYPE(ep));
